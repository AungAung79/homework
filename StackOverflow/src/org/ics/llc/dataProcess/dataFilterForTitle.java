...
public class dataFilterForTitle {
	...
	public void scan()
	{
		File file=new File("parsed-data/QuestionsTitles.txt");
		...
		try {
			...
			File stat=new File("processed-data/wordFreForTitle.txt");
			...
			for(int i=dic.size()-1;i>=0;i--)
			{
				...
				if(dicHashSet.get(heheString)<=2)
				{
					dic.remove(i);
			...
			stat=new File("processed-data/wordFreRemoved20ForTitle.txt");
	...
	{
		File file=new File("parsed-data/QuestionsTitles.txt");
		File pro=new File("processed-data/QuestionsTitlesProcessed.txt");
		String line=new String();
		try {
			BufferedReader reader=new BufferedReader(new InputStreamReader(new FileInputStream(file)));
	...
	{
		dataFilterForTitle df=new dataFilterForTitle();
		df.scan();
		df.rescan();
	}
}
