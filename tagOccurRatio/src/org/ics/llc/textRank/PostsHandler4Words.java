...
public class PostsHandler4Words extends DefaultHandler {
	...
	public void startDocument() throws SAXException {
		try {
			...
			outTitle = new FileOutputStream("data/QuestionsTitles.txt");
			psTitle = new PrintStream(outTitle);
			
			outTag = new FileOutputStream("data/QuestionsTags.txt");
			psTag = new PrintStream(outTag);
			
			outBody = new FileOutputStream("data/QuestionsBody.txt");
	...
	public void startElement(String uri, String localName, String qName,
		...
		if(Integer.parseInt(attributes.getValue(1)) == 1) {// questions
			...
			for (int i = 0; i < attributes.getLength(); i++) {
				...
				{
					try {
						outTemp = new FileOutputStream("data/FileTemp.txt");
						psTemp = new PrintStream(outTemp);
						psTemp.print(attributes.getValue(i).toLowerCase());
						PTBTokenizer ptbt;
						ptbt = new PTBTokenizer(new FileReader("data/FileTemp.txt"),new CoreLabelTokenFactory(),"");
